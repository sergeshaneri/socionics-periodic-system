...
export default function App() {
  ...
  const getTraitExplanation = (traitIdx: number, itemIdx: number) => {
    ...
    if (objectType === 'TIM') {
      ...
      fullTrait = poles[traitIdx] ? `${poles[traitIdx][0]}/${poles[traitIdx][1]}` : (traits[traitIdx] || '');
      ...
      if (lang === 'RU') {
        if (activeTIM.id === 'СЭИ' && traitIdx === 2 && bit === 1) {
          description = "Дюма чаще рассчитывают на то, что все может быть так как раньше не было, надеются новому опыту. Не то чтобы они не опираются на прошлый опыт, просто они не зациклены на нем.";
        } else if (activeTIM.id === 'ЭИИ' && traitIdx === 4 && bit === 0) {
          description = "Достоевские имеют тенденцию обращать внимание на характеристики объектов, которые обусловлены принадлежностями к группам. Это можно назвать “инклюзивные” характеристики. Они включены в свойства объекта и всех других объектов, которые принадлежат к группе.";
        }
      }
    } 
      ...
      fullTrait = poles[itemIdx] ? `${poles[itemIdx][0]}/${poles[itemIdx][1]}` : (traits[itemIdx] || '');
      ...
      if (lang === 'RU') {
        // Democrat/Aristocrat (index 4), EII (index 15)
        if (itemIdx === 4 && traitIdx === 15) {
          description = "Достоевские имеют тенденцию обращать внимание на характеристики объектов, которые обусловлены принадлежностями к группам. Это можно назвать “инклюзивные” характеристики. Они включены в свойства объекта и всех других объектов, которые принадлежат к группе.";
        }
        // Extravert/Introvert (index 1), LSE (index 14)
        if (itemIdx === 1 && traitIdx === 14) {
          description = "Экстраверты более шумные и активные, легко тратят энергию, заводят новые знакомства. Конкретно про Штира можно сказать, что он довольно глубоко считывает внешнюю информацию о деятельности и технологиях.";
        }
      }
    }
    else {
      ...
      fullTrait = poles[traitIdx] ? `${poles[traitIdx][0]}/${poles[traitIdx][1]}` : (traits[traitIdx] || '');
      description = lang === 'RU' ? "Описание интертипного отношения в данной мерности." : "Description of the intertype relation in this dimension.";
    }
  ...
  const activeTrait = hoveredTraitIdx !== null && hoveredIdx !== null 
    ? getTraitExplanation(hoveredTraitIdx, hoveredIdx) 
    : null;
